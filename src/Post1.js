...
const Post1 = () => {
  ...
  <div className="absolute  top-0 w-screen flex flex-col items-center justify-start gap-[46px] ">
    ...
    <div className="w-[716px] flex flex-col items-start justify-start gap-[43px]">
      ...
      <div className="text-[20px] font-['Pretendard'] font-medium text-[#000] whitespace-nowrap">특히 이 곳의 특별한 점은, 마스코트 태릉이가 반겨준다는 것!<br/>모른척 할 일을 하다보면 슬쩍 팬서비스를 해주고 가니 먼저 다가오기를 기다려보자.<br/> </div>
      <div className="text-[20px] font-['Pretendard'] font-medium text-[#000] whitespace-nowrap">마스트리스보트의 시그니처 커피는 보트커피와 액설런트 라떼이다.<br/>그리고 이 고소한 커피들에 꾸덕한 티라미수를 함께 곁들여 먹는 것을 추천한다.<br/><br/>커피와 티라미수의 달콤 쌉싸름한 조합이 이 카페의 분위기와도 잘 어우러진다.<br/>티라미수는 얼그레이, 쑥, 로투스 등 다양한 맛이 있으니 취향에 맞게 골라먹는 재미도!</div>
      <div className="w-[698px] text-[20px] font-['Pretendard'] font-medium text-[#000]">조용하게 시간을 보내기에도, 혼자 작업을 하기에도 적합한 따뜻한 공간이지만<br/>곳곳의 고양이 폴라로이드와 손그림으로 그려진 메뉴들처럼 애정이 담긴 것들을 찬찬히 둘러보는 것도 권한다.<br/>일기장을 챙겨간다면 떠오르는 영감들을 차곡차곡 쌓을 수 있을 것이다.</div>
      <div className="w-[698px] text-[20px] font-['Pretendard'] font-medium text-[#000]">테이블 이용 시간은 3시간으로 제한되고 있고, 터줏대감 ‘태릉이’가 계시는 만큼 강아지님들은 방문이 어려운 점 꼭 참고해<br/>카페를 넘어선 하나의 고유한 공간, 마스트리스 보트에서 다들 각자만의 시간을 즐기고 오길.</div>
    </div>
  </div>
</div>)
}
